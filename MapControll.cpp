...
MapControll::MapControll(int width, int height, int x, int y, int map, std::shared_ptr<Player> allen, std::shared_ptr<PartyControll> pc) :
	positionPlayer{ x, y },
	directionPlayer(DirectionPlayer::FRONT),
...
void MapControll::SetMap() {
	...
	for (int x = 0; x < mapSize.width; x++) {
		// 村マップはサイズで制限をかける
		if (x < 20) {
			village_map_data.push_back(*new std::vector<std::shared_ptr<Map>>);
		}
		world_map_data.push_back(*new std::vector<std::shared_ptr<Map>>);
		for (int y = 0; y < mapSize.height; y++) {
			std::shared_ptr<Map> temp(new Map());
			//村マップはサイズで制限をかける
			if (x < 20) {
				village_map_data[x].push_back(temp);
			}
			world_map_data[x].push_back(temp);
		}
	}

	// メインマップのデータをキャッシュする
	{
		std::ifstream stream("datas\\worldmap.csv");
		std::string line;
		...
		while (getline(stream, line)) {
			...
			for (std::string::size_type spos, epos = 0;
				...
				world_map_data[col][row]->setData(stoi(token));
				world_map_data[col][row]->setTable(table);
				col++;
			}
			++row;
		}
	...
	//村マップのデータをキャッシュする
	{
		std::ifstream stream("datas\\test_map.csv");
		std::string line;
		const std::string delim = ",";
...
void MapControll::createMap(ID id) {
	...
	case MAP_NORMAL:
		...
		std::copy(world_map_data.begin(), world_map_data.end(), maps.begin());
		break;
	case MAP_HAJIET:
		...
		std::copy(village_map_data.begin(), village_map_data.end(), maps.begin());
		break;
	}
